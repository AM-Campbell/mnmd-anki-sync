...
### Fixed
- **Scope modifiers now work correctly**: `[-1]`, `[1]`, `[-1,1]` properly include surrounding paragraphs
- **Clean context expansion**: When scope pulls in other paragraphs, any cloze syntax in those paragraphs is cleaned up (shows the answer, not raw `{{...}}` syntax)

### Changed
- Updated documentation to clarify that `> ?` blocks are optional
- Paragraphs containing clozes automatically become flashcards
- Improved syntax guide with clearer examples

## [0.1.1] - 2025-11-29

### Fixed
- Fixed lint issues for CI
- Updated syntax command to show man-page style documentation
